...
setwd("C:/Users/ameli/Dropbox/UW SAFS/Converse Lab/Lab Meetings/2020_Spring/Week 8")
library(MuMIn)
...
# Use dredge to fit all possible models
dfm1.logit.jack <- dredge(fm1.logit, rank="AIC") # do these need to be ranked?

# Get all of the model fit results
fm1mods.logit <- get.models(dfm1.logit.jack, subset = TRUE)
...
# Fit the candidate models, omitting one data point at a time.
N <- nrow(logitdat)
M <- length(fm1mods.logit)
train <- logitdat # we will use the entire dataset here, rather than a subset
J <- matrix(NA, N, M) # matrix with jackknifed predictions
for (i in 1:N){
  # re-fit models on train with one less data point:
  jfits <- lapply(fm1mods.logit, update, .~. , data=train[-i,]) 
  # predict them to omitted data point:
  J[i,] <- sapply(jfits, predict, newdata=train[i, , drop=F]) 
  rm(jfits)
}
